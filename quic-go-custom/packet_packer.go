...
// ==========================================================================
// 👑 GEN ĐỘ TỐI CAO: Khóa cứng mốc 1050 bytes cho cả chiều dội data về từ Server
// ==========================================================================
var HuangCustomMTU int = 1050
...
func (p *packetPacker) packConnectionClose(
	...
	if HuangCustomMTU > 0 && int(maxPacketSize) > HuangCustomMTU {
		// fmt.Printf("[DEBUG][SERVER-CORE-MTU] 🎪 packConnectionClose: Ép hạ trần từ %d xuống %d bytes\n", maxPacketSize, HuangCustomMTU)
...
func (p *packetPacker) PackCoalescedPacket(onlyAck bool, maxSize protocol.ByteCount, now monotime.Time, v protocol.Version) (*coalescedPacket, error) {
	...
	if HuangCustomMTU > 0 && int(maxSize) > HuangCustomMTU {
		// fmt.Printf("[DEBUG][SERVER-CORE-MTU] 📡 PackCoalescedPacket: Khống chế tối đa từ %d xuống %d bytes\n", maxSize, HuangCustomMTU)
...
func (p *packetPacker) appendPacket(
	...
	if HuangCustomMTU > 0 && int(maxPacketSize) > HuangCustomMTU {
		// // fmt.Printf("[DEBUG][SERVER-CORE-MTU] 🚀 appendPacket (Application Data): Bóp kích thước từ %d xuống %d bytes\n", maxPacketSize, HuangCustomMTU)
		maxPacketSize = protocol.ByteCount(HuangCustomMTU)
	}
...
func (p *packetPacker) PackPTOProbePacket(
	...
	if HuangCustomMTU > 0 && int(maxPacketSize) > HuangCustomMTU {
		// fmt.Printf("[DEBUG][SERVER-CORE-MTU] 🔍 PackPTOProbePacket: Điều tiết từ %d xuống %d bytes\n", maxPacketSize, HuangCustomMTU)
		maxPacketSize = protocol.ByteCount(HuangCustomMTU)
	}
